...
const Header = ({ className = "" }) => {
  return (
    <header
      style={{flexWrap: "wrap"}}
      className={`self-stretch min-h-[60px] rounded-21xl bg-dark-grey box-border flex flex-row items-center justify-between p-2 top-[0] z-[99] sticky gap-[20px] max-w-full text-left text-29xl text-white font-space-grotesk border-[1px] border-solid border-gray-100 ${className}`}
    >
      ...
      <div className="self-stretch w-[200px] flex flex-row items-center justify-end">
        <Button
          className="self-stretch justify-between pr-[6px] w-36"
          endIcon={
            <div className="flex items-center justify-center rounded-full bg-[#222] w-[38px] h-[38px]">
                <img width="24px" height="24px" src="/arrow-outward.svg" />
              </div>
              }
          disableElevation
          variant="contained"
